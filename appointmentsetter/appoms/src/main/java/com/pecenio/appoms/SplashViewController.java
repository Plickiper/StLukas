...
public class SplashViewController implements Initializable {
	...
	public void onClicked(ActionEvent actionEvent)  throws IOException {
		...
		window.hide();
		if(manageScene == null){
			FXMLLoader fxmlLoader = new FXMLLoader(ManageAppointmentSetterJFXApp.class.getResource("manage-appo-view.fxml"));
			Parent root = (Parent)fxmlLoader.load();
			ManageAppointmentSetterController controller = fxmlLoader.getController();
			controller.setStage(stage);
			manageScene = new Scene(root, 300, 800);
			controller.setSplashScene(splashScene);
			controller.setManageScene(manageScene);
		}
		stage.setTitle("AppointmentSetter Management");
